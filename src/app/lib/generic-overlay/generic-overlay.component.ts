...
import { transformOverlay } from './generic-overlay.animations';

export type OverlayPositionX = 'before' | 'center' | 'after';
export type OverlayPositionY = 'above'  | 'center' | 'below';

@Component({
  selector: 'sat-generic-overlay',
  exportAs: 'satGenericOverlay',
  encapsulation: ViewEncapsulation.None,
  animations: [transformOverlay],
  styleUrls: ['./generic-overlay.component.scss'],
  template: `
    <ng-template>
      <div class="sat-generic-overlay"
        [ngClass]="classList"
        #focusTrapElement
...
export class SatGenericOverlayComponent implements AfterViewInit {

  /** Position of the overlay on the x axis. */
  @Input()
  get xPosition() { return this._xPosition; }
  set xPosition(val: OverlayPositionX) {
    this._xPosition = val;
  ...
  set yPosition(val: OverlayPositionY) {
    this._yPosition = val;
    this.setPositionClasses();
  }
  ...
  private _xPosition: OverlayPositionX = 'center';

  /** Private y position store overloaded by input setter/getter. */
  private _yPosition: OverlayPositionY = 'center';
